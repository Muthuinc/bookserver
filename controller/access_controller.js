...
exports.verifyLogin = async (req, res) => {
  try {
    ...
    const employee = await Restaurant.findOne({ username });

    if (employee) {
      const isPasswordValid = password === employee.password; 
      if (isPasswordValid) {
        const token = jwtToken.sign({ id: employee._id }, process.env.SECRET_KEY, { expiresIn: "30d" });
        ...
        const mailFormat = {
          ...
          html: `<h4>Hai dear,</h4><br><p>Welcome back to Bromag India! Use the link to access your dashboard: <a href="${dashboardLink}">${dashboardLink}</a></p>`,
        };

        sendEmail(mailFormat.to, mailFormat.subject, mailFormat.html);
        return res.json({ success: true, token, message: `Welcome ${employee.username}!` });
      } else {
    ...
    return res.status(500).json({ success: false, serverMessage: "Internal Server Error" });
  }
};
